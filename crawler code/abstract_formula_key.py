...
class FormulaSpider(object):
    ...
    def parse_formula(self, html, url):
        ...
        for sub_list in self.url_dic[url]:
            for item in self.url_dic[url][sub_list]:
                formula_list.append(item)
        formula_list = [i.replace(' ', '').replace("\n", "") for i in formula_list]
        ...
        # latex_dic去除
        latex_dic = {key: value for key, value in latex_dic.items() if
                     key.replace(' ', '').replace("\n", "") in formula_list}
        # add去重
        add_list = [item for item in add_list if
                    item.replace(' ', '').replace("\n", "") not in [i.replace(' ', '').replace("\n", "") for i in
                                                                    latex_dic]]
        add_dic = {item: {"before": "", "after": ""} for item in add_list}
        # text_list若存在于latex_list，去重
        text_list = [item for item in text_list if
                     item.replace(' ', '').replace("\n", "") not in [i.replace(' ', '').replace("\n", "") for i in
                                                                     latex_dic]]
    ...
    def get_latex_selectolax(html):
        ...
        dl_dic = {**dl_dic, **{key: value for key, value in where_dic.items() if key not in where_dic}}
        dl_dic = {**dl_dic, **{key: value for key, value in annotation_dic.items() if key not in where_dic}}
        return dl_dic
        ...
        for tag in tree.css("annotation"):
            if "=" in tag.text(strip=True) and "\\displaystyle" in tag.text(strip=True):
                ...
                if next_node is not None:
                    ...
                    after += i
                dic[index] = {"before": before, "after": after}
        return dic
    ...
    def process(self):
        # 读取url_list
        f = open('output/ver4.0_filiter_formula_list_10.json', 'r', encoding='utf-8')
        content = f.read()
...
spider.get_html("https://en.wikipedia.org/wiki/Net_national_income")
'''
f0 = open('economics_formula_list_1.json', 'r', encoding='utf-8')
content = f0.read()
...
f2 = open('economics_formula_list_2.json', 'w', encoding='utf-8')
f2.write(temp)
f2.close()
f0 = open('economics_formula_list_2.json', 'r', encoding='utf-8')
content = f0.read()
b = json.loads(content)
f0 = open('output/ver4.0_filiter_formula_list_10.json', 'r', encoding='utf-8')
content = f0.read()
a = json.loads(content)
new_dict = {key: b[key] for key in a if key in b}
temp = json.dumps(new_dict, ensure_ascii=False)
f2 = open('economics_formula_list_3.json', 'w', encoding='utf-8')
f2.write(temp)
f2.close()'''
